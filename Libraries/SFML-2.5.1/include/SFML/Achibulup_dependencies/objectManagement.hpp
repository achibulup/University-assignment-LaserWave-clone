...
/// UPDATE: due to change in C++ rules, this class is redundant and become an alias to pointer
#if false //ACHIBULUP__Cpp17
template<typename Tp>
class RelaxedPtr
{
    using voidptr = void*;
...
#else //false //ACHIBULUP_Cpp17_prior

template<typename Tp>
using RelaxedPtr = Tp*;

#endif //false //ACHIBULUP_Cpp17_prior

template<typename Tp = void>
RelaxedPtr<Tp> blessedNew(size_t s)
{
    if(s < 0) throw std::bad_alloc();
    return s ? static_cast<Tp*>(static_cast<void*>(new Byte[s * ssizeof<Tp>()])) 
             : nullptr;
}
inline RelaxedPtr<void> blessedNew(size_t s)
{
    return blessedNew<Byte>(s);
}
template<typename Tp>
void blessedDelete(RelaxedPtr<Tp> ptr)
{
...
#if false //ACHIBULUP__Cpp17_prior
template<typename Tp>
...
{
    Destructor<Tp>::destroy(RelaxedPtr<Tp>(pos));
}
template<typename Tp>
void destroy(Tp *pos, size_t num) noexcept
{
    Destructor<Tp>::destroy(RelaxedPtr<Tp>(pos), num);
}
...
class Buffer
{
  public:
    ...
    : size(len), m_buffer(pointer(blessedNew(len))) {}
