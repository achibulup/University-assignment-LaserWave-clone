...
template<typename Tp, typename = EnableIf_t<std::is_trivial<Tp>::value>>
inline ACHIBULUP__constexpr_fun14 Tp& MoveAssign(Tp &dest, Tp &src) noexcept
{
    dest = src;
    src = {};
    return dest;
...
inline ACHIBULUP__constexpr_fun14 Tp& MoveAssign(Tp &dest, Tp &src) 
        noexcept(noexcept(dest = Move(src)))
{
    dest = Move(src);
    return dest;
}
...
class AutoProperty
{
  private:
    ...
    using MoveRef = n_utils::MoveReference<AutoProperty>;

    explicit ACHIBULUP__constexpr_fun14 AutoProperty(MoveRef other) noexcept 
    : value(Move(other.ref.value)) {}

    ACHIBULUP__constexpr_fun14
    AutoProperty& operator = (MoveRef other) noexcept
    {
        MoveAssign(this->value, other.ref.value);
        return *this;
    }

    friend constexpr n_utils::MoveReference<AutoProperty> 
    Move(AutoProperty &src) noexcept
    {
        return {std::move(src)};
    }
  ...
  public:
    ...
    ACHIBULUP__constexpr_fun14
    const Type& operator = (Type value) & noexcept
    {
        return this->value = value;
    }
    
    ACHIBULUP__constexpr_fun14 
    const Type& operator ++ () & noexcept
    {
        return ++this->value;
    }
    ACHIBULUP__constexpr_fun14 
...
class AutoProperty<const Type, Class>
{
  private:
    ...
    explicit ACHIBULUP__constexpr_fun14 AutoProperty(MoveRef value) noexcept 
    : value(Move(value.ref.value)) {}
    
    ACHIBULUP__constexpr_fun14 
    AutoProperty& operator = (MoveRef value) & noexcept
    {
        MoveAssign(this->value, value.ref);
        return *this;
    }

    friend constexpr MoveRef Move(AutoProperty &src) noexcept
    {
        return {std::move(src)};
    }
